...
void main() {
  ...
  group('UpdateLeadRequest', () {
    test('always sends contactName, because @Valid rejects a body without it',
        () {
      final json = const UpdateLeadRequest(contactName: 'Rehana Akter').toJson();
      expect(json['contactName'], 'Rehana Akter');
    });

    test('omits untouched fields so the sparse patch leaves them alone', () {
      final json = const UpdateLeadRequest(
        contactName: 'Rehana Akter',
        // Blank is the same as absent here: the service treats a missing key
        // as "leave it", and sending '' would overwrite a real value.
        companyName: '   ',
      ).toJson();
    ...
    test('sends the fields that were filled in', () {
      final json = const UpdateLeadRequest(
        contactName: '  Rehana Akter  ',
      ...
      expect(json['estimatedValue'], 25000);
    });
  });

  group('OpportunityRequest', () {
    // The backend assigns description and nextStep unconditionally on update,
    // so an omitted key erases the stored value rather than preserving it.
    // These two tests are the guard on that.
    test('sends description and nextStep even when they are empty', () {
      final json = const OpportunityRequest(
        name: 'Annual licence renewal',
        description: '',
        nextStep: null,
      ).toJson();
      ...
      expect(json['nextStep'], isNull);
    });

    test('seeds both from the deal so an edit does not blank them', () {
      const existing = Opportunity(
        ...
        probability: 60,
        createdAt: '2026-08-01T10:00:00',
        description: 'Renewal for 240 seats',
        nextStep: 'Send the revised quote',
      );

      final json = OpportunityRequest.from(existing).toJson();

      expect(json['description'], 'Renewal for 240 seats');
      expect(json['nextStep'], 'Send the revised quote');
    });

    test('omits clientId when it is not set, so update ignores it', () {
      final json = const OpportunityRequest(
        name: 'Annual licence renewal',
        description: null,
        nextStep: null,
  ...
  group('CreateClientRequest', () {
    test('never provisions a portal login by accident', () {
      final json = const CreateClientRequest(
        firstName: 'Rehana',
        lastName: 'Akter',
        email: 'rehana@example.com',
      ).toJson();

      expect(json['provisionPortalLogin'], isFalse);
    });
  });

  group('UpdateClientRequest', () {
    test('seeds from the client and drops what is blank', () {
      const client = Client(
        id: 7,
        status: 'ACTIVE',
        createdAt: '2026-08-01T10:00:00',
      ...
      final json = UpdateClientRequest.from(client).toJson();

      expect(json['clientCompanyName'], 'Dhrubotara Ltd');
