...
void main() {
  ...
  group('UpdateTicketRequest', () {
    test('always sends the three the endpoint replaces outright', () {
      final json = const UpdateTicketRequest(
        title: '  Cannot log in  ',
        description: '  Since this morning.  ',
        priority: 'HIGH',
      ).toJson();

      expect(json['title'], 'Cannot log in');
      expect(json['description'], 'Since this morning.');
      expect(json['priority'], 'HIGH');
    });

    test('omits the category, which is the one guarded field', () {
      final json = const UpdateTicketRequest(
        title: 'Cannot log in',
        description: 'Since this morning.',
        priority: 'HIGH',
    ...
    test('deleting is a narrower set of roles than editing', () {
      for (final role in ticketDeleteRoles) {
        expect(ticketEditRoles, contains(role));
      }
      expect(ticketEditRoles, contains('SUPPORT_AGENT'));
      expect(ticketDeleteRoles, isNot(contains('SUPPORT_AGENT')));
    });
  });
}
