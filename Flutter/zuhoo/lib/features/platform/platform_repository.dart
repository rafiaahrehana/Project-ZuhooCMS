...
import '../finance/finance_models.dart' show CreateExpenseRequest, Expense;
...
class PlatformRepository {
  ...
  ///
  /// A duplicate email is refused with "Email already exists"; an
  /// out-of-range role with "Invalid platform role selected". Both are worth
  /// showing as written.
  Future<PlatformUser> createUser(CreatePlatformUserRequest request) async {
    final json =
        await _api.post<Map<String, dynamic>>(_users, request.toJson());
    return PlatformUser.fromJson(json);
  }

  /// Defines a new subscription plan. Refused when the code is already taken.
  Future<SubscriptionPlanOption> createPlan(
  ...
  ///
  /// Every field is required except phone and password — see
  /// [UpdatePlatformUserRequest]. A blank password leaves their login alone.
  ...
  //
  // Same DTO and same response shape as a company's expenses, so the models
  // are reused wholesale from the finance module rather than duplicated. What
  // differs is the path and the roles.

  static const _platformExpenses = '/platform/finance/expenses';

  /// The whole ledger, or one slice of it.
  ///
  /// Both filters are **path segments** rather than query parameters, and only
  /// one can apply at a time — which is why the caller passes at most one.
  Future<PagedResponse<Expense>> platformExpenses({
    String? status,
    String? vendor,
    bool mineOnly = false,
    int page = 0,
    ...
    final path = switch ((status, vendor, mineOnly)) {
      (final s?, _, _) => '$_platformExpenses/status/$s',
      (_, final v?, _) => '$_platformExpenses/vendor/${Uri.encodeComponent(v)}',
      (_, _, true) => '$_platformExpenses/my-expenses',
      _ => _platformExpenses,
    };
    return _api.getPaged(path, Expense.fromJson, page: page, size: size);
  }

  Future<Expense> createPlatformExpense(CreateExpenseRequest request) async {
    final json = await _api.post<Map<String, dynamic>>(
      _platformExpenses,
      request.toJson(),
    );
  ...
  /// Approving takes notes as a **required** query parameter — the controller
  /// declares `@RequestParam String notes` with no `required = false`, so an
  /// empty one is a 400 rather than an approval without a comment.
  Future<void> approvePlatformExpense(int id, String notes) => _api.post<dynamic>(
        '$_platformExpenses/$id/approve'
        '?notes=${Uri.encodeQueryComponent(notes.trim())}',
      );

  /// Same shape, and the reason is equally required.
  Future<void> rejectPlatformExpense(int id, String reason) => _api.post<dynamic>(
        '$_platformExpenses/$id/reject'
        '?reason=${Uri.encodeQueryComponent(reason.trim())}',
      );

  /// Records the reimbursement. Both parameters are optional here.
  Future<void> markPlatformExpensePaid(
    int id, {
    String? method,
    String? reference,
  }) {
    ...
    return _api.post<dynamic>('$_platformExpenses/$id/mark-as-paid$query');
  }

  /// Removes it outright. SUPER_ADMIN alone — see
  /// [platformExpenseDeleteRoles]. Rejecting is almost always the right action
  /// instead, which is why the screen offers this last.
  Future<void> deletePlatformExpense(int id) =>
      _api.delete<dynamic>('$_platformExpenses/$id');
}
...
class PlatformUsersController extends AsyncNotifier<PagedState<PlatformUser>>
  ...
  Future<PlatformUser> create(CreatePlatformUserRequest request) async {
    final created =
        await ref.read(platformRepositoryProvider).createUser(request);
    await refresh();
    return created;
  }
