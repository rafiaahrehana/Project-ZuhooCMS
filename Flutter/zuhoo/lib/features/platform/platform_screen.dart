...
class PlatformScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // The platform's own books. Narrower again, and a different set from the
    // rest — an accountant reaches this and nothing else on the console.
    final canExpenses = user?.hasAnyRole(platformExpenseRoles) ?? false;

    final labels = <String>[
      'Companies',
      'Flags',
      if (canStaff) 'Staff',
      if (canContext) 'Context',
      if (canAudit) 'Audit',
      if (canExpenses) 'Expenses',
    ];

    // Defining a plan and adding staff are both SUPER_ADMIN / SYSTEM_ADMIN,
    // the same gate as the staff directory itself. A plan belongs to the
    // Companies tab because that is where plans get assigned.
    final createOn =
      ...
      child: Builder(
        builder: (context) {
          ...
          return AnimatedBuilder(
            ...
            builder: (context, _) {
              final action = createOn[labels[tabController.index]];
              return Scaffold(
                backgroundColor: bos.bgPage,
                appBar: AppBar(
                  title: const Text('Platform'),
                  bottom: TabBar(
                    // Five tabs do not fit a phone width evenly; scrolling
                    // beats three-letter truncations of "Companies".
                    isScrollable: true,
                ...
                body: TabBarView(
                  children: [
                    ...
                    if (canExpenses) const PlatformExpensesTab(),
                  ],
                ),
              );
            },
      ...
      itemBuilder: (context, user) => AppCard(
        child: Row(
          children: [
            ...
            // Editing is the same gate as the list itself — reaching this tab
            // already means SUPER_ADMIN or SYSTEM_ADMIN.
            IconButton(
