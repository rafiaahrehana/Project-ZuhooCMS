...
/// The platform's own books. `PlatformExpenseController` is
/// `PLATFORM_ACCOUNTANT` or `SUPER_ADMIN` on every method, except delete,
/// which is `SUPER_ADMIN` alone.
const platformExpenseRoles = <String>[
  'PLATFORM_ACCOUNTANT',
  'SUPER_ADMIN',
];

/// Deleting an expense outright, rather than rejecting it.
const platformExpenseDeleteRoles = <String>['SUPER_ADMIN'];

/// PATCH /platform/users/{id}
///
/// Not the create DTO: the password is optional here — blank means "leave
/// their login alone" — while everything else is required, and the backend
/// checks each one by hand with a readable message rather than through
/// validation.
///
/// The role must be one of [assignablePlatformRoles]; anything else is refused
/// with "Invalid platform role selected".
class UpdatePlatformUserRequest {
  const UpdatePlatformUserRequest({
  ...
  /// Null or blank leaves the existing password in place. Anything else
  /// replaces it, which signs the person out of nothing but changes what they
  /// must type next time.
  final String? password;

  factory UpdatePlatformUserRequest.from(PlatformUser user) =>
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      ...
      // Not trimmed: whitespace is part of a password. Omitted entirely when
      // empty, which is what tells the backend not to reset it.
...
/// The billing cycles a subscription plan can run on.
const billingCycles = <String>['MONTHLY', 'QUARTERLY', 'YEARLY', 'ONE_TIME'];

/// POST /platform/users
///
/// Creates a member of platform staff and the login they sign in with. The
/// role must be one of [assignablePlatformRoles] — the service checks it
/// against the same list and answers "Invalid platform role selected"
/// otherwise, so the form only ever offers those.
class CreatePlatformUserRequest {
  const CreatePlatformUserRequest({
    required this.firstName,
    required this.lastName,
    required this.email,
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      ...
      'lastName': lastName.trim(),
      'email': email.trim(),
      // Not trimmed: whitespace is part of a password.
      'password': password,
      'role': role,
...
///
/// The code is the plan's identity and is upper-cased server-side before the
/// uniqueness check, so the form upper-cases it too rather than letting a
/// lower-case entry look different from what will be stored.
class CreateSubscriptionPlanRequest {
  const CreateSubscriptionPlanRequest({
    required this.code,
    required this.name,
