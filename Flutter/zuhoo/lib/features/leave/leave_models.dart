/// The types someone may **apply** for. Deliberately shorter than the
/// backend's `LeaveType`: EMERGENCY and OTHER exist there but are not things a
/// person picks off a list when booking time off.
...
/// Every type the backend knows, for HR configuring a balance against one.
/// A balance can exist for a type nobody self-serves.
const allLeaveTypes = <String>[...leaveTypes, 'EMERGENCY', 'OTHER'];
...
///
/// All four fields are `@NotNull` and the endpoint is `@Valid`, so there is
/// nothing optional here — a balance is an employee, a type, a year and a
/// number of days, or it is nothing.
///
/// `totalDays` is a whole number server-side (`Integer`), not the fractional
/// day count the balance reads back as.
