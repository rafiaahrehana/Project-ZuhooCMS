...
class LeaveRepository {
  ...
  /// Grants somebody an entitlement for a year.
  ///
  /// Creating a second balance for the same employee, type and year is
  /// refused by the backend rather than merged, so the caller shows that
  /// message rather than a generic failure.
  Future<LeaveBalance> createBalance(LeaveBalanceRequest request) async {
...
class LeaveController extends AsyncNotifier<LeaveState> {
  ...
  Future<LeaveState> _load() async {
    ...
    // if requestsCall throws first, this function returns before balancesCall
    // is awaited, and a call nobody is still waiting on reports its own
