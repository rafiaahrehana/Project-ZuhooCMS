...
abstract final class PayrollPermissions {
  /// Reading the run, the dashboard and everybody's lines.
  static const view = 'PAYROLL_VIEW';

  /// One code covers running payroll and creating a single record by hand.
  /// Also opening, recalculating, submitting and cancelling a run.
  static const process = 'PAYROLL_PROCESS';

  /// Signing a run off, rejecting it, and paying it. A separate code from
  /// [process] on purpose — whoever prepares payroll should not also approve
  /// it, and the backend enforces that split.
  static const approve = 'PAYROLL_APPROVE';
}

/// POST /hr/payroll
///
/// Creates one month's record for one employee by hand. The batch equivalent
/// is `POST /hr/payroll/generate`, which builds a draft for everybody with a
/// salary structure — that is the monthly routine, and this is the exception
/// to it: a new joiner, a correction, somebody the run missed.
///
/// Employee, month and year are `@NotNull` and the endpoint is `@Valid`, so
/// those three always go. Every figure is optional: `resolveSalaryComponents`
/// falls back to the employee's salary structure for anything absent, so an
/// omitted allowance means "use the structure", not "zero". The net is never
/// sent — the backend derives it from attendance, overtime, billable hours,
/// loan repayments and extra components, and a phone guessing at it would
/// disagree with the payslip.
///
/// A second record for the same employee and period is refused, not merged.
///
/// The DTO also accepts food and special allowances and three named deductions
/// (tax, insurance, provident fund). Those are left off this form deliberately:
/// eleven money fields do not belong on a phone, and each one omitted still
/// resolves from the structure. The full breakdown stays on the web.
