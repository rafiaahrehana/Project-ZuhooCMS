...
abstract final class RequestPermissions {
  ...
  /// Editing a request is `checkAnyPermission(ASSIGN, APPROVE, CLOSE)` — any
  /// one of the three will do, so all three are named here and the UI asks for
  /// the same set rather than a single code the backend never checks alone.
  static const assign = 'SERVICE_REQUEST_ASSIGN';
  static const close = 'SERVICE_REQUEST_CLOSE';

  static const editAny = [assign, approve, close];

  /// The only permission in the service-package family. Creating one is
...
/// A file attached to a request — anything from `POST /upload` handed to
/// `POST /service-requests/{id}/documents` for the record to point at.
...
///
/// A true sparse patch — every field is null-checked before it is assigned —
/// so an omitted key is left alone. Refused once the request is closed
/// (`guardNotClosed`), which is what [RequestStatus.isOpen] gates on.
///
/// The DTO also accepts `slaDeadline`, `assignedEmployeeId` and the two
/// government reference fields. Assigning somebody is its own flow with its own
/// endpoint, and an SLA deadline is set by policy rather than by hand, so this
/// form offers neither.
class UpdateServiceRequestRequest {
  const UpdateServiceRequestRequest({
    this.title,
    this.description,
    this.priority,
    this.agreedPrice,
  ...
  factory UpdateServiceRequestRequest.from(ServiceRequest request) =>
...
/// The billing cycles a service package can run on.
const packageBillingCycles = <String>[
  'MONTHLY',
  'QUARTERLY',
  'YEARLY',
  'ONE_TIME',
...
///
/// A bundle of catalogue services sold on a cycle. Name and billing cycle are
/// required; `requestQuota` is `@Min(1)` when given, so it is either a real
/// allowance or absent — never zero.
///
/// Four fields are assigned **unconditionally** by the update method and so are
/// always sent, seeded from the current package when editing:
///
/// * `packagePrice` — deliberately so, per the backend's own comment: a null
///   clears a manual override and returns the package to its calculated price.
/// * `discountPercent` — a null is coerced to zero, so omitting it wipes the
///   discount rather than leaving it alone.
/// * `featured` and `popular` — primitive booleans, which Jackson defaults to
///   false when the key is absent.
///
/// `serviceIds` is the opposite: null-guarded, so omitting it keeps whatever
/// services are bundled. That also means an empty list here preserves the
/// bundle rather than clearing it — the app cannot empty a package, which is
/// the safer of the two failure modes.
///
/// The DTO also carries Bengali name and description fields, an icon URL and
/// terms. Those are storefront copy, edited where the storefront is, and this
...
class CreateServicePackageRequest {
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      ...
      // The four unconditional fields, always present: see the class comment.
      ...
      if (requestQuota != null) 'requestQuota': requestQuota,
...
/// POST /reviews
///
/// A portal client rating a service request they raised. `submitOrUpdate` —
/// posting again replaces the earlier rating rather than adding a second, so
/// the sheet never has to ask whether one already exists.
///
/// Refused unless the request is COMPLETED and belongs to the caller, and the
/// endpoint is `hasRole('CLIENT')` — staff cannot review on a client's behalf.
class SubmitReviewRequest {
  ...
  /// 1–5. Anything outside is refused with a message naming the range.
  final int rating;
...
class ServiceReview {
  ...
  /// Whether the company has chosen to show it publicly. A client's own rating
  /// is theirs either way; this only decides whether anyone else sees it.
