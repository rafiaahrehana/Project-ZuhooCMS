    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            // The catalogue is what requests are raised against, so this is the
            // closest home the app has for defining a package. Gated on the
            // package permission rather than shown to every employee, even
            // though the backend itself only checks the role.
            if (permissions.has(RequestPermissions.packageView))
              IconButton(
                icon: const Icon(Icons.inventory_2_outlined),
                tooltip: 'New package',
                onPressed: () => showNewPackageSheet(context),
              ),
          ],
          bottom: TabBar(
