...
/// Editing and deleting a ticket are gated on **roles**, not permission codes
/// — the only two endpoints in this module that are. There is no
/// `TICKET_UPDATE` to check, so the app matches the controller's own
/// `@PreAuthorize` lists rather than inventing a code the backend would ignore.
const ticketEditRoles = <String>[
  'SUPPORT_MANAGER',
  'SUPPORT_AGENT',
  'COMPANY_OWNER',
  'SUPER_ADMIN',
...
/// Narrower than [ticketEditRoles]: an agent may edit a ticket but not delete
/// one.
const ticketDeleteRoles = <String>[
  'COMPANY_OWNER',
  'SUPER_ADMIN',
  'SYSTEM_ADMIN',
];
...
///
/// Reuses the create DTO server-side, and it is `@Valid`: title, description
/// and priority are all required, and all three are assigned unconditionally.
/// So this is a replace, not a patch — the form seeds from the ticket and
/// sends the whole thing back.
///
/// `categoryId` is the exception: it *is* null-checked, so leaving it unset
/// keeps whatever category the ticket already had.
class UpdateTicketRequest {
  ...
  factory UpdateTicketRequest.from(SupportTicket ticket) =>
      UpdateTicketRequest(
        title: ticket.title,
