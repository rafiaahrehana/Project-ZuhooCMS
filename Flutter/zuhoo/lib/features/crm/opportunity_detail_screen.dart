...
class _OpportunityDetailScreenState
  ...
  /// Removes the deal and leaves the screen.
  Future<void> _delete(Opportunity opportunity) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Delete ${opportunity.name}?'),
        content: const Text(
          'The deal stops counting towards the pipeline and its forecast.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
    ...
    try {
      await ref.read(opportunitiesProvider.notifier).delete(opportunity.id);
      if (!mounted) return;
      navigator.pop();
      messenger.showSnackBar(const SnackBar(content: Text('Deal deleted.')));
    } on ApiException catch (e) {
      messenger.showSnackBar(SnackBar(content: Text(e.message)));
    } catch (_) {
      messenger.showSnackBar(
        const SnackBar(content: Text('Could not delete that deal.')),
      );
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  ...
  Widget build(BuildContext context) {
    ...
    // A won or lost deal is refused by the backend's update path, so editing
    // is offered only while the deal is still open. Deleting a closed deal is
    // still allowed.
    final canEdit = loaded != null &&
        Stage.isOpen(loaded.stage) &&
        permissions.has(CrmPermissions.opportunityUpdate);
    final canDelete =
        loaded != null && permissions.has(CrmPermissions.opportunityDelete);
...
///
/// A separate widget so the callbacks close over a non-nullable [Opportunity].
class _OpportunityMenu extends StatelessWidget {
  const _OpportunityMenu({
  ...
  final bool enabled;
  final Future<void> Function(Opportunity) onDelete;
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      onSelected: (value) {
        if (value == 'edit') {
          showEditOpportunitySheet(context, opportunity);
      ...
      itemBuilder: (context) => [
        if (canEdit)
          const PopupMenuItem(value: 'edit', child: Text('Edit deal')),
        if (canDelete)
