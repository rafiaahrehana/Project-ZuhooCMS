...
/// The one thing a tab's floating button creates.
class _TabAction {
  const _TabAction({
    required this.label,
    required this.icon,
...
class CrmScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Each tab carries its own primary action, so the button over the list
    // always creates the thing that list is made of. The pipeline has none:
    // a deal needs a client, so it is opened from one — see
    // `showNewOpportunitySheet`.
    final tabs = <({String label, Widget view, _TabAction? action})>[
      if (permissions.has(CrmPermissions.opportunityView))
        (label: 'Pipeline', view: const PipelineTab(), action: null),
      if (permissions.has(CrmPermissions.leadView))
        (
          label: 'Leads',
          view: const LeadsTab(),
          action: permissions.has(CrmPermissions.leadCreate)
              ? const _TabAction(
                  label: 'New lead',
                  icon: Icons.person_add_alt_rounded,
                  open: showNewLeadSheet,
                )
              : null,
        ),
      if (permissions.has(CrmPermissions.clientView))
        (
          label: 'Clients',
          view: const ClientsTab(),
          action: permissions.has(CrmPermissions.clientCreate)
              ? const _TabAction(
                  label: 'New client',
    ...
    return DefaultTabController(
      ...
      child: Builder(
        builder: (context) {
          ...
          // button floating over the clients list would create the wrong thing.
          final tabController = DefaultTabController.of(context);
          return AnimatedBuilder(
            animation: tabController,
            builder: (context, _) {
              final action = tabs[tabController.index].action;
              return Scaffold(
                backgroundColor: bos.bgPage,
                appBar: AppBar(
