...
class _LeadDetailScreenState extends ConsumerState<LeadDetailScreen> {
  ...
  /// Removes the lead and leaves the screen, because there is nothing left
  /// here to look at once it is gone.
  Future<void> _delete(Lead lead) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Delete ${lead.contactName}?'),
        content: const Text(
          'The lead and its activity history stop appearing anywhere in CRM.',
        ),
        actions: [
          TextButton(
    ...
    try {
      await ref.read(leadsProvider.notifier).delete(lead.id);
      if (!mounted) return;
      navigator.pop();
      ...
      messenger.showSnackBar(
        const SnackBar(content: Text('Could not delete that lead.')),
      );
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Built from whatever is already loaded: with no lead there is nothing to
    // act on, and the menu is left off rather than shown with dead entries.
    final lead = async.value;
    final canEdit = lead != null &&
        canEditLead(lead) &&
        permissions.has(CrmPermissions.leadUpdate);
    final canDelete = lead != null && permissions.has(CrmPermissions.leadDelete);
...
///
/// A separate widget so the callbacks close over a non-nullable [Lead] — the
/// screen only knows it has one once the detail request has resolved.
class _LeadMenu extends StatelessWidget {
  const _LeadMenu({
    required this.lead,
  ...
  final bool enabled;
  final Future<void> Function(Lead) onDelete;
