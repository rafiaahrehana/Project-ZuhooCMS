...
class AttendanceScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Assigning a shift is HR work on somebody else, so it sits in the app bar
    // rather than in the tabs, which are about the reader's own attendance.
    final canAssignShift = ref
          ...
          actions: [
            if (canAssignShift)
              IconButton(
                ...
                tooltip: 'Assign a shift',
                onPressed: () => showAssignShiftSheet(context),
              ),
          ],
          bottom: const TabBar(
            tabs: [Tab(text: 'Overview'), Tab(text: 'Timesheets')],
          ),
        ),
        body: const TabBarView(
          children: [_OverviewTab(), TimesheetsTab()],
        ),
      ),
    );
  }
...
class _OverviewTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // `.value` rather than `.when(...)`: a shift card is supplementary
    // context for a screen whose primary content is the punch card and the
    // month summary below, not something worth an error banner of its own.
    // Absent covers both "not on a shift" and "could not load it right now"
    // — indistinguishable here on purpose, the same way the leave-balance
    // context on the approvals tab treats a failed fetch as just not shown.
    final shift = ref.watch(myShiftAssignmentProvider).value;
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(Icons.wb_twilight_rounded, size: 18, color: bos.brandInk),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (shift.assignmentStartDate != null)
                  Text(
                    'Since ${Fmt.date(shift.assignmentStartDate)}',
