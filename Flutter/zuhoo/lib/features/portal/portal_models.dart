...
class InvoiceItem {
  ...
  /// Computed server-side by `calculateLineTotal`, never sent back.
  final double? lineTotal;

  final String? notes;

  double get amount => lineTotal ?? quantity * unitPrice;
...
class Invoice {
  ...
  /// Parsed so an edit can send it back. `ClientInvoiceServiceImpl.update`
  /// assigns it unconditionally, so an edit that omitted it would silently
  /// clear the rate the invoice was raised at.
  final double? taxRatePercent;

  /// Same reason, more sharply: the update endpoint is `@Valid` against the
  /// create DTO, whose `items` is `@NotEmpty`. So an edit has to resend the
  /// lines even though the service would have left them alone.
  ...
  /// Only a draft can be edited or deleted — the backend refuses anything else
  /// with "Only DRAFT invoices can be updated".
...
/// PATCH /clients/me
///
/// A true sparse patch — `updateMyProfile` null-checks every field, so an
/// omitted key is left alone rather than blanked. `shippingAddress` is one the
/// endpoint accepts but this app never sends: the profile response does not
/// carry it, so the form has nothing to show and nothing to send back.
class UpdateMyProfileRequest {
  ...
  factory UpdateMyProfileRequest.from(ClientProfile profile) =>
      UpdateMyProfileRequest(
        clientCompanyName: profile.clientCompanyName,
