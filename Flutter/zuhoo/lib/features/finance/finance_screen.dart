...
import '../receivables/receivables_repository.dart'
    show ReceivablesRepository, receivablesRepositoryProvider;
          ...
          return AnimatedBuilder(
            ...
            builder: (context, _) {
              return Scaffold(
                ...
                // Each tab creates the thing it lists — a "claim an expense"
                // button floating above the invoice ledger would create the
                // wrong thing.
                floatingActionButton: tabs[tabController.index].showsFab
                    ...
                    : tabs[tabController.index].label == 'Invoices' &&
                            canRaiseInvoice
                        ? FloatingActionButton.extended(
...
class _InvoiceCardState extends ConsumerState<_InvoiceCard> {
  ...
  Future<void> _delete(Invoice invoice) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text('Delete ${invoice.invoiceNumber}?'),
        content: const Text('The draft stops appearing anywhere in finance.'),
        actions: [
          TextButton(
    ...
    try {
      ...
      messenger.showSnackBar(
        const SnackBar(content: Text('Could not delete that invoice.')),
      );
    } finally {
  ...
  /// Emails the invoice to the client, or cancels it.
  ///
  /// Both answer with an empty body, so the list is reloaded rather than
  /// patched — sending moves a draft to issued and cancelling moves it out of
  /// what is owed.
  Future<void> _act(
  ...
  Widget build(BuildContext context) {
    ...
    final canEdit = permissions.has(FinancePermissions.invoiceUpdate);
    // Deleting is `hasRole('COMPANY_OWNER')` at the controller, so the
    // permission alone is not enough to complete it.
    final canDelete = permissions.has(FinancePermissions.invoiceDelete) &&
        (user?.isCompanyOwner ?? false);
    final canRecordPayment = permissions.has(FinancePermissions.receiptCreate);
