...
class ApiClient {
  ...
  /// `POST /api/upload` — the one generic document endpoint every feature's
  /// attachment capability sends through, so the 10MB cap and content-type
  /// checking only live in one place on the backend and one call here.
  Future<UploadedFile> uploadDocument(String filePath, String fileName) =>
      _guard(() async {
