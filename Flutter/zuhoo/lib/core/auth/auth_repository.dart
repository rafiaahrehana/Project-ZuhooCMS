...
class AuthRepository {
  ...
  /// Creates a brand-new tenant company plus its owner account. The account is
  /// unusable until the code emailed to it is entered — see [verifyEmail] —
  /// so this deliberately does not sign anyone in.
  Future<RegisteredUser> register(RegisterRequest request) async {
    final json =
        await _api.post<Map<String, dynamic>>('/auth/register', request.toJson());
    return RegisteredUser.fromJson(json);
  }

  Future<String> verifyEmail(String email, String code) => _api.postText(
        '/auth/verify-email',
        VerifyEmailRequest(email: email, code: code).toJson(),
      );

  /// Always 200 whether or not the address is registered — same
  /// anti-enumeration behaviour as the password-reset endpoints.
  Future<String> resendVerification(String email) => _api.postText(
        '/auth/resend-verification',
        ResendVerificationRequest(email).toJson(),
      );
