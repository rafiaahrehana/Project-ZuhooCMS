...
class Env {
  ...
  static String get host {
    ...
    // A release build with no override would otherwise fall through to the
    // emulator/localhost address below and silently ship talking to a host
    // that does not exist on the customer's phone — a confusing "can't
    // connect" with no clue why, discovered after the build is already out.
    // Failing loudly here, at first use, is a build-config mistake caught in
    // testing rather than a support ticket.
    if (kReleaseMode) {
      throw StateError(
        'API_BASE_URL was not set for this release build. Build with '
        '--dart-define=API_BASE_URL=https://your-production-backend/api',
      );
    }
