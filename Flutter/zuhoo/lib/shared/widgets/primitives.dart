...
/// Grows a fixed-height horizontal strip — a row of filter chips, typically —
/// so its one line of text does not clip when the reader has turned their
/// system text size up. Same growth curve `home_screen.dart`'s stat grid uses
/// for the same reason, generalised to any base height.
double scaledStripHeight(BuildContext context, double base) {
  final scale = MediaQuery.textScalerOf(context).scale(1);
  return base + (scale - 1).clamp(0.0, 1.5) * (base * 0.5);
}
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (onDismiss != null)
            // A visually small icon still needs a ~44dp tap box — shrinking
            // `constraints` down to the icon's own bounds (as this used to)
            // makes it very hard to hit accurately, especially for anyone
